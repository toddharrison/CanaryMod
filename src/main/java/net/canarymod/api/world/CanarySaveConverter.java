    ...
    public void convert() {
        ...
        if (netherWorld != null) {
            if (netherWorld.renameTo(new File(baseFolder, baseFolder.getName() + "_" + "NETHER"))) {
                log.info("Nether converted successfully!");
            }
            else {
                log.warn("Failed to convert the Nether!");
            }
        }
        else {
            log.info("No Nether World to convert.");
        }

        if (endWorld != null) {
            if (endWorld.renameTo(new File(baseFolder, baseFolder.getName() + "_" + "END"))) {
                log.info("The End converted successfully!");
            }
            else {
                log.warn("Failed to convert the End!");
            }
        }
        else {
            log.info("No End World to convert.");
        }
        ...
        log.info("If you see no Warnings above, conversion was successful!");
    }
}
